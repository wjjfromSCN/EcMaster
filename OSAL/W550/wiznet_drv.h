 ...
 */
INT32 wiznet_hw_config(uint32_t spi_clkDiv /*default 8 ~ 31.25 MHz*/, 
					INT32 startup_rst /*default 1 (enable)*/, 
					uint32_t link_wait_us /*default 1000000 (1s)*/);

INT32 wiznet_macraw_init(uint8_t *enetaddr);
INT32 wiznet_macraw_send(void *packet, INT32 length);
INT32 wiznet_macraw_recv(uint8_t * packet, size_t size);
